...
const TestimonialCard: FC<TestimonialCardProps> = ({
  ...
  return (
    <motion.div
      ...
      className={`cursor-pointer bg-white shadow-lg ring-1 ring-gray-200 rounded-lg overflow-hidden p-6 transition-all z-20 ${
        expanded ? 'z-10 col-span-3 scale-[1.02]' : 'hover:shadow-md'
      } ${isDimmed ? 'opacity-50 scale-95' : ''}`}
    >
      <motion.div layout='position' className='relative h-8 w-28 mb-4'>
        <Image src={logo} alt={name} fill className='object-contain' />
      </motion.div>
      <motion.p layout='position' className='mb-4 text-gray-700 text-sm italic'>
        “{quote}”
      </motion.p>
      <motion.div
        layout='position'

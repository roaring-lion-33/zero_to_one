...
const WhoItsFor: FC = () => {
  return (
    ...
    >
      ...
      >
        Whether you’re shaping a vision or scaling your ops — we’re here to help
      ...
      >
        {items.map(({ title, text, icon: Icon }, index) => (
          ...
          >
            ...
            <h3 className='mb-2 text-lg font-semibold text-[#030b1a]'>
              {title}
            </h3>
            <RevealText className='text-sm text-gray-600'>{text}</RevealText>
          </motion.div>
        ))}
      </motion.div>
    </MotionSection>

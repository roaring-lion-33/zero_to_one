...
export default function WhyWeExist() {
  // ✅ Declare refs and visibility flags outside the map
  const [ref0, inView0] = useInView({ threshold: 0.3, triggerOnce: false });
    ...
    >
        ...
        <div className='relative z-20 space-y-12 border-r-2 border-transparent'>
          {items.map((text, i) => (
            ...
            >
              ...
              />
              <RevealText className='mr-6 text-lg text-gray-700 leading-relaxed'>
                {text}
              </RevealText>
            </motion.div>
          ))}
      ...
      >
        ...
        <p>
          ...
          deals, and scales fast. We’ve seen what happens when great ideas never
          get the right launch — when timelines slip, teams fumble, or
          confidence fades. That’s why we care so deeply about getting it right
          from day one.
        </p>
        <div className='flex mt-6 justify-end text-gray-500 font-medium'>
          – Frank
        </div>
      </motion.blockquote>
    </MotionSection>

...
const steps = [
  {
    ...
    description:
      'We clarify your idea, define your MVP, and establish the technical plan for a fast, scalable build.',
  },
  {
    icon: Wrench,
    title: 'Days 31–60: Design & Build',
    description:
      'We design clean, user-focused interfaces and develop your app using scalable, maintainable code.',
  },
  {
    icon: Rocket,
    title: 'Days 61–90: Launch & Iterate',
    description:
      'We help you launch confidently, gather user feedback, and iterate for traction and growth.',
  },
];
...
export default function LaunchPlanSteps() {
  return (
    <MotionSection
      ...
      whileTap={{ scale: 0.98 }}
      className='py-24 px-6 bg-white/10'
      id='launch-plan'
    >
        ...
        <ol className='relative border-r border-gray-200'>
          {steps.map(({ icon: Icon, title, description }, index) => (
            ...
            >
              ...
              <h3 className='mb-1 text-xl font-semibold text-[#030b1a]'>
                {title}
              </h3>
              <RevealText className='text-lg text-gray-600'>
                {description}
              </RevealText>
            </motion.li>
          ))}

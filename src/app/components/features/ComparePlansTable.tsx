...
const tiers = [
  {
    ...
    icon: Hammer,
    description: 'Fast foundation for early founders.',
  },
  {
    id: 'sprint',
    name: '60-Day MVP Sprint',
    icon: Rocket,
    recommended: true,
    description: 'Full MVP with Stripe, PDF, and CI/CD.',
  },
  {
    id: 'launchpack',
    name: '90-Day Launch Pack',
    icon: Star,
    description: 'Includes monetization, onboarding, and analytics.',
  },
  {
    id: 'studio',
    name: '180-Day Studio',
    icon: Layers3,
    description: 'All-in-one studio build with support + branding.',
  },
] as const;
...
const features = [
    ...
    items: [
      {
        ...
        icon: Hammer,
        includedIn: ['kickstart', 'sprint', 'launchpack', 'studio'],
      },
      {
        label: 'UI/UX Design',
        icon: Star,
    ...
    items: [
      {
        ...
        icon: Rocket,
        includedIn: ['sprint', 'launchpack', 'studio'],
      },
      {
    ...
    items: [
      {
        ...
        icon: Star,
        includedIn: ['launchpack', 'studio'],
      },
      {
        label: 'Advanced Reports + PDF Templates',
        icon: Star,
        includedIn: ['launchpack', 'studio'],
      },
      {
        label: 'Customer Portal & Analytics',
        icon: Star,
        includedIn: ['launchpack', 'studio'],
      },
    ],
  },
  {
    ...
    items: [
      ...
      {
        label: 'Multi-Tenant SaaS + Roles',
        icon: Layers3,
        includedIn: ['studio'],
      },
      {
        label: 'Collab Tools + Notifications',
        icon: Layers3,
        includedIn: ['studio'],
      },
    ],
      ...
      <motion.table className='w-full text-left border-collapse rounded-xl shadow-xl overflow-hidden'>
        <thead>
          <tr className='bg-[var(--accent)] text-white text-sm'>
            <th className='sticky left-0 z-10 px-4 py-4 bg-[#030b1a] text-white'>
              Features
            </th>
            {tiers.map(tier => (
              <th
          ...
          {features.map(({ category, items }) => (
            <Fragment key={`category-${category}`}>
              <tr className='bg-gray-50 text-xs uppercase tracking-wide font-semibold text-gray-600'>
                <td className='sticky left-0 z-10 px-4 py-3 bg-white'>
                  {category}
                </td>
                {tiers.map(tier => (
                  <td
                    key={`category-${category}-col-${tier.id}`}
                    className='bg-white'
                  />
                ))}
              </tr>

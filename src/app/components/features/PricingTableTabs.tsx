  ...
  return (
    <MotionSection className='px-6 py-24 bg-white' id='pricing'>
      <div className='max-w-6xl mx-auto mb-16 text-center'>
        <RevealText className='mb-2 text-3xl font-bold tracking-tight text-[#030b1a]'>
          Pricing Packages
        </RevealText>
        <p className='max-w-xl mx-auto text-gray-600'>
          Choose a package that fits your stage — or let’s customize one
          together.
        </p>
      </div>
      ...
      <div className='grid grid-cols-1 max-w-6xl mx-auto gap-10 sm:grid-cols-2 lg:grid-cols-3'>
        {tiers.map((tier, i) => (
          ...
          >
            {tier.highlight && (
              <div className='mb-4 text-xs font-semibold text-blue-600 uppercase'>
                Most Popular
              </div>
            )}
            ...
            <RevealText className='mb-4 text-gray-500 text-sm'>
              {tier.description}
            </RevealText>
            <div className='mb-6 text-3xl font-bold text-[#030b1a]'>
              {tier.price}
            </div>
